...
func main() {
	flag.Parse()
	if *showVersion {
		fmt.Printf("protoc-gen-uniform %v\n", version)
		return
	}
...
func generateFile(p *protogen.Plugin, f *protogen.File) {
	...
	// Generate new file
	g := p.NewGeneratedFile(f.GeneratedFilenamePrefix+".pb.uniform.go", f.GoImportPath)
	g.P("// Code generated by protoc-gen-uniform. DO NOT EDIT.")
	g.P("// versions:")
	protocVersion := "(unknown)"
	if v := p.Request.GetCompilerVersion(); v != nil {
		protocVersion = fmt.Sprintf("v%d.%d.%d", v.GetMajor(), v.GetMinor(), v.GetPatch())
	}
	// Set version of protoc-gen-uniform and protoc
	g.P("// \tprotoc-gen-uniform: ", version)
